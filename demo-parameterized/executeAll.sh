...
INITIALVERSION="49f75e8877c2e9b7cf6b56087121a35fdd73ff8b"
...
STATE=$(grep -A21 '"call" : "de.dagere.peass.Callee#innerMethod",' "results/$VERSION/de.dagere.peass.ExampleTest_test(JUNIT_PARAMETERIZED-1).js" \
    | grep '"state" : "SLOWER",' \
    | grep -o 'SLOWER')
if [ "$STATE" != "SLOWER" ]
then
    ...
    cat "results/$VERSION/de.dagere.peass.ExampleTest_test(JUNIT_PARAMETERIZED-1).js"
    exit 1
else
    echo "Slowdown is detected for Callee#innerMethod."
fi

SOURCE_METHOD_LINE=$(grep "Callee.method1_" "results/$VERSION/de.dagere.peass.ExampleTest_test(JUNIT_PARAMETERIZED-1).js" -A 3 \
    | head -n 3 \
    | grep innerMethod)
